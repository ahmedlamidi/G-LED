...
def projection(img_array, spacing):
    ...
    angles_deg = np.arange(0, 360, 0.1, dtype=np.float32)
   ...
   # 1. Create a "Master" Geometry for the full Sinogram
# This is only used to tell ASTRA how to filter the data
    pg_full = astra.create_proj_geom('fanflat', dx, num_detectors, angles, DSO, ODD)
    sino_id_full = astra.data2d.create('-sino', pg_full, sino_AD)

# 2. Apply the Filter to the entire Sinogram at once
# We use 'FP_CUDA' and then extract the filtered result
# Or simpler: use ASTRA's built-in filtering tool
    recon_accumulator = np.zeros((H, W), dtype=np.float32)
    chunk_size = 100

    for i in range(0, num_angles, chunk_size):
        end_idx = min(i + chunk_size, num_angles)

        curr_angles = angles[i:end_idx]
        curr_sino = np.ascontiguousarray(
            sino_AD[i:end_idx, :],
            dtype=np.float32
        )

        pg_chunk = astra.create_proj_geom(
            'fanflat',
            dx,
            num_detectors,
            curr_angles,
        ...
        vg_chunk = astra.create_vol_geom(
            H, W,
            -W*dx/2, W*dx/2,
            -H*dy/2, H*dy/2
        )

        s_id = astra.data2d.create('-sino', pg_chunk, curr_sino)
        v_id = astra.data2d.create('-vol', vg_chunk)

        cfg = astra.astra_dict("FBP_CUDA")   # 🔥 NOT BP_CUDA
        cfg["ProjectionDataId"] = s_id
        cfg["ReconstructionDataId"] = v_id

        alg_id = astra.algorithm.create(cfg)
        astra.algorithm.run(alg_id)

    # weight contribution properly
        recon_accumulator += astra.data2d.get(v_id) * (len(curr_angles) / num_angles)

        astra.algorithm.delete(alg_id)
        astra.data2d.delete(s_id)
        astra.data2d.delete(v_id)

    return recon_accumulator
