...
for batch_name in batch_folders:
    ...
    try:
        ...
        # Asymmetric conjugate split (matching train_diff)
        H_gt = img_ground_truth.shape[0]
        half = H_gt // 2
        cond_indices = [i for i in range(0, half, 4)] + [i for i in range(half + 2, H_gt, 4)]
        cond_set = set(cond_indices)
        label_indices = [i for i in range(H_gt) if i not in cond_set]
