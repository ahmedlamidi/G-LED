...
def process_dicom(dicom_path, output_root, *, limit_deg, index_step,
    ...
    # Degraded sinogram (limited-angle + sparse)
    ...
    subdir = f"FBP_limited{limit_deg:g}deg_step{step_deg:g}deg_{len(keep_idx)}views"
    ...
    recon_diff = np.abs(full_rec_hu - masked_rec_hu)
    ...
    axes[1, 1].imshow(masked_rec_hu, cmap="gray", vmin=vmin, vmax=vmax)
    axes[1, 1].set_title(f"Reconstruction – {mode_title}")
    axes[1, 1].axis("off")
    ...
    fig.suptitle(
        f"Comparison (W={window_width}, L={window_level})\n"
    ...
    panels = {
        ...
        "recon_masked": (masked_rec_hu, f"Reconstruction – {mode_title}",
                         dict(cmap="gray", vmin=vmin, vmax=vmax, aspect="equal")),
    ...
    # ── standalone masked reconstruction ─────────────────────────────────
    single_path = os.path.join(out_dir, f"{stem}_reconstruction_masked_only.pdf")
    plt.figure(figsize=(8, 8))
    plt.imshow(masked_rec_hu, cmap="gray", vmin=vmin, vmax=vmax)
    plt.axis("off")
    ...
    np.save(os.path.join(out_dir, f"{stem}_recon_masked_hu.npy"), masked_rec_hu)

    print(f"  [{stem}] Saved to {out_dir}  ({len(keep_idx)} views kept)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch FBP degradation: limited-angle + sparse within 0-N° range."
    )
